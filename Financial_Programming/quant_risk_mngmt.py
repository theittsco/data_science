...
Risk management: Reduce effects of adverse outcomes
Quantifiable uncertainty: Identity factors to measure risk
...
Voltility via covariance matrix
'''
...
###############################################################################
'''
Risk factors and the 2008 Financial crisis

Volatility: Measures the dispersion of returns around expected value

Time series: Expected value = sample average

What drives expectation and dispersion? Risk factors
...
Risk exposure: Measure of possible portfolio loss
    Example: Flood insurance
        ...
        -Deductible is risk exposure

Systematic risk: Risk factors affecting volatility of all portfolio assets
Market risk: Systematic risk from general financial market movements

Systematic risk examples: Inflation, interest rate changes, economic climate
changes

Idiosyncratic risk: Risk specific to a particular asset/asset class
Examples:
    -Bond portfolio: Issuer at risk of default
    -Firm/sector characteristics: Firm size (market capitalization), book to
     market ratio, sector shocks

Factor models: Assessment of risk factors affecting portfolio return

Regression: OLS
    -Dependent variable: Returns or volatility
    -Independent variable(s): Systematic and idiosyncratic risk factors
Fama-French model: Combo of market risk and idiosyncratic risk
...
-Intuitively: Greater uncertainty compensated by greater return
-CANNOT garantee return: need some measure of expected return
    -Average historical return -> proxy for expected future return

Minimum return required for given level of risk? -> (risk,return) dataset

Choosing portfolio weights
-Vary portfolio weights of given portfolio -> cretes set of (risk,return) points
-Changing weights = beginning risk management
GOAL: Change weights to maximize expected return, given risk level
      OR minimize risk given expected return
Changing weights = adjusting investor's risk exposure

Efficient portfolio: Portfolio with weights generating highest expected return
...
'''
The average historical return is usually available as a proxy for expected
returns, but is not always accurate--a more thorough estimate of expected
returns requires an assumption about the return distribution.

Portfolio optimization relies upon an unbiased and efficient estimate of asset
covariance. Although sample covariance is unbiased, it is not efficient--extreme
events tend to be overweighted.

One approach to alleviate this is through "covariance shrinkage", where large
errors are reduced ('shrunk') to improve efficiency.
'''
...
'''
Although the differences between the sample covariance and the efficient
covariance (found by shrinking errors) may seem small, they have a huge impact
on estimation of the optimal portfolio weights and the generation of the
efficient frontier. Practitioners generally use some form of efficient
covariance for Modern Portfolio Theory.
'''
...
Maximum Loss:
-What's the maximum loss of a portfolio?
-Losses cannot be bounded with 100% certainty
-Confidence Level: Replace 100% certainty with likelihood of upper bound

Measured using VaR - Statistic measuring maximum loss at a particular confidence
level

CVaR: Measures expected loss given a minimum loss equal to the VaR, expected
value of the tail of the loss distribution
'''
...
'''
The conditional value at risk (CVaR), or expected shortfall (ES), asks what the
average loss will be, conditional upon losses exceeding some threshold at a
certain confidence level. It uses VaR as a point of departure, but contains more
information because it takes into consideration the tail of the loss distribution.
'''
...
'''
Although VaR and CVaR are similar (and only one letter apart!), it's generally
the case that CVaR is the preferred risk measure for risk management. One reason
is that it is affected by the tail of the loss distribution, while VaR is a
static value.

CVaR is an expected value over all of the losses exceeding the VaR, which are
precisely the tail.
'''
...
2. Loss associiated with shock - VaR, CVaR
3. Desire to avoid shock - Risk tolerance

Risk exposure: Probability of loss times loss measure (VaR)
'''
...
Hedging: Offset voltility with another asset

Hedging Instrument: Options
-Derivative:
...
This is because an option only needs to be exercised when it is profitable to do
so, which means that more volatility increases the chance for profit. In the
next exercise, you'll examine how an option's profitability changes instead with
the price of the underlying stock over time.
'''
...
'''
Suppose that you have an investment portfolio with one asset, IBM. You'll hedge
the portfolio's risk using delta hedging with a European put option on IBM.

First, value the European put option using the Black-Scholes option pricing
formula, with a strike X of 80 and a time to maturity T of 1/2 a year. The
risk-free interest rate is 2% and the spot S is initially 70.

Then create a delta hedge by computing the delta of the option with the
bs_delta() function, and use it to hedge against a change in the stock price to
69.5. The result is a delta neutral portfolio of both the option and the stock.
'''
...
-Class of possible distributions
    Class of distributions f(x,theta)
    x is loss (random variable)

Fit distribution according to error-minimizing
...
'''
Your test statistic was well above the critical F-value, indicating that a
structural break in the data occurred in the summer of 2008.
'''
...
Rolling window voltility: Compute volatility

Extreme values:
Incorporates VaR, CVaR
-Useful for large data sets
Alternative: Find losses exceeding some threshold

Backtesting: Use CVaR
'''
...
the estimate of the 95% VaR from 2009-2010 data is not robust across the possible
structural break identified in the previous exercise. Backtesting remains an
important verifier of risk measure estimates such as VaR.
'''
...
Non-parametric estimation: Does not assume a distribution such as normal,
skewed normal, or t dist.
'''
